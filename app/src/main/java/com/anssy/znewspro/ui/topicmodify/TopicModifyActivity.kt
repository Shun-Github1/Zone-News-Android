    ...
    private fun initModel(){
        ...
        topicModel.topicListEntry.observe(this){
            if (it!=null){
                if (it.code== Constants.SUCCESS_CODE){
                    ...
                    SharedPreferenceUtils.putList(mContext,getString(R.string.topic_modify_shared_pref_list_key),mList)
        ...
        topicModel.commonResponseEntry.observe(this){
            if(it!=null){
                if (it.code== Constants.SUCCESS_CODE){
                    setResult(RESULT_OK)
                    TipDialog.show(getString(R.string.topic_modify_delete_success),WaitDialog.TYPE.SUCCESS)
                    mList.remove(it.msg)
                    mAdapter.notifyDataSetChanged()
                }else{
                    if (it.code==1000){
                        TipDialog.show(getString(R.string.topic_modify_server_error_dialog),WaitDialog.TYPE.ERROR)
                    }else{
        ...
        mAdapter = object : CommonAdapter<String>(this,R.layout.item_topic_recycler,mList){
            override fun convert(holder: ViewHolder, t: String, position: Int) {
                ...
                deleteIv.setOnClickListener {
                    BottomMenu.show(arrayOf(getString(R.string.topic_modify_delete),getString(R.string.topic_modify_cancel)))
                        .setMenuTextInfo(TextInfo().setGravity(Gravity.CENTER))
                        .setOnMenuItemClickListener { dialog, text, index ->
                            dialog.dismiss()
                            if (index==0){
                                WaitDialog.show(getString(R.string.topic_modify_submitting))
