...
object SystemDialogUtils {
    ...
    fun showLoadingDialog(context: Context, message: String) {
        ...
        // Constraint width to 85% of screen width or 400dp
        loadingDialog?.window?.let { window ->
            val metrics = context.resources.displayMetrics
            val screenWidth = metrics.widthPixels
            val maxWidthPx = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 400f, metrics).toInt()
            val targetWidth = minOf((screenWidth * 0.85).toInt(), maxWidthPx)
    ...
    fun showAlertDialog(
        ...
        // Fix for dialog being too wide on some devices
        // Constraint width to 85% of screen width or 400dp, whichever is smaller
        dialog.window?.let { window ->
            val metrics = context.resources.displayMetrics
            val screenWidth = metrics.widthPixels
            val maxWidthPx = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 400f, metrics).toInt()
